-- =============================================
-- 🔥 ZaZaHub v2.0 для FPS Flick
-- С вкладками, исправленным аимом и прицелами
-- =============================================
...
-- =============================================
-- 📦 ГЛАВНОЕ МЕНЮ (горизонтальные вкладки)
-- =============================================
...
-- Контейнер для вкладок (горизонтальные)
...
-- Контент для каждой вкладки (переключается)
...
-- Функция создания кнопок внутри вкладок
...
local function canSee(targetPart)
    ...
    if hit then
        local hitParent = hit:IsDescendantOf(targetPart.Parent)
        return hitParent
    end
    return false
end
    ...
    for player, data in pairs(espPlayers) do
        if player and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if onScreen and espEnabled then
                data.box.Position = Vector2.new(pos.X - 30, pos.Y - 45)
...
local rainbowSpeed = 5  -- скорость перелива (чем выше, тем быстрее)
...
-- Слайдер для скорости перелива (упрощённо кнопками +/-)
...
createButton(visTab, "⬆ Увеличить скорость", 0.45, function()
    rainbowSpeed = math.min(rainbowSpeed + 1, 20)
    updateSpeedLabel()
end)

createButton(visTab, "⬇ Уменьшить скорость", 0.55, function()
...
local function rotateCrosshair()
    ...
    for _, obj in pairs(crosshairParts) do
        if obj:IsA("Line") then
            local center = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
            -- Получаем текущие точки линии
            local from = obj.From
            local to = obj.To
            -- Вычисляем середину линии
            local mid = (from + to) / 2
            -- Вектор от середины к точке
            local vec1 = from - mid
            local vec2 = to - mid
...
print("🔥 ZaZaHub v2.0 загружен!")
print("✅ Аимбот с проверкой видимости")
print("✅ ESP (квадраты + линии)")
print("✅ Радужные руки с регулируемой скоростью")
print("✅ 3 типа вращающихся прицелов")
